...
export function createSourceFileNode(node: ASTNode, ast: SourceFileAST): ts.SourceFile {
  ...
  if (node.children) {
    for (const childId of node.children) {
      ...
      // 特殊处理 SyntaxList：提取其中的语句
      if (childNode.kind === ts.SyntaxKind.SyntaxList) {
        if (childNode.children) {
          for (const stmtId of childNode.children) {
            const stmtNode = ast.nodes[stmtId];
            if (!stmtNode) continue;
            
            const tsNode = createTSNode(stmtNode, ast);
            if (ts.isStatement(tsNode)) {
              statements.push(tsNode);
            } else if (ts.isExpression(tsNode)) {
              statements.push(ts.factory.createExpressionStatement(tsNode));
            }
          }
        }
      } else {
